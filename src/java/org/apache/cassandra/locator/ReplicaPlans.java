...
public class ReplicaPlans
{
    ...
    public static ReplicaPlan.ForReadRepair forReadRepair(ClusterMetadata metadata, Keyspace keyspace, ConsistencyLevel consistencyLevel, Token token, Predicate<Replica> isAlive) throws UnavailableException
    {
        ...
        EndpointsForToken candidates = candidatesForRead(consistencyLevel, forTokenRead.natural());
...
//            throw UnavailableException.create(ConsistencyLevel.ONE, 1, 1, 0, 0);
    ...
    {
        return forRead(ClusterMetadata.current(), keyspace, token, consistencyLevel, retry, false);
    }

    public static ReplicaPlan.ForTokenRead forRead(ClusterMetadata metadata, Keyspace keyspace, Token token, ConsistencyLevel consistencyLevel, SpeculativeRetryPolicy retry)
    {
        return forRead(metadata, keyspace, token, consistencyLevel, retry, true);
    }

    private static ReplicaPlan.ForTokenRead forRead(ClusterMetadata metadata, Keyspace keyspace, Token token, ConsistencyLevel consistencyLevel, SpeculativeRetryPolicy retry,  boolean throwOnInsufficientLiveReplicas)
    {
        AbstractReplicationStrategy replicationStrategy = keyspace.getReplicationStrategy();
        ReplicaLayout.ForTokenRead forTokenRead = ReplicaLayout.forTokenReadLiveSorted(metadata, keyspace, replicationStrategy, token);
        EndpointsForToken candidates = candidatesForRead(consistencyLevel, forTokenRead.natural());
        ...
        return new ReplicaPlan.ForTokenRead(keyspace, replicationStrategy, consistencyLevel, candidates, contacts,
                                            (newClusterMetadata) -> forRead(newClusterMetadata, keyspace, token, consistencyLevel, retry, false),
    ...
    {
        return forRangeRead(ClusterMetadata.current(), keyspace, consistencyLevel, range, vnodeCount, true);
    }

    public static ReplicaPlan.ForRangeRead forRangeRead(ClusterMetadata metadata, Keyspace keyspace, ConsistencyLevel consistencyLevel, AbstractBounds<PartitionPosition> range, int vnodeCount, boolean throwOnInsufficientLiveReplicas)
    {
        AbstractReplicationStrategy replicationStrategy = keyspace.getReplicationStrategy();
        ReplicaLayout.ForRangeRead forRangeRead = ReplicaLayout.forRangeReadLiveSorted(metadata, keyspace, replicationStrategy, range);
        EndpointsForRange candidates = candidatesForRead(consistencyLevel, forRangeRead.natural());
        ...
        return new ReplicaPlan.ForRangeRead(keyspace,
                                            ...
                                            contacts,
                                            vnodeCount,
                                            (newClusterMetadata) -> forRangeRead(newClusterMetadata, keyspace, consistencyLevel, range, vnodeCount, false),
    ...
    public static ReplicaPlan.ForRangeRead maybeMerge(Keyspace keyspace, ConsistencyLevel consistencyLevel, ReplicaPlan.ForRangeRead left, ReplicaPlan.ForRangeRead right)
    {
        ...
        return new ReplicaPlan.ForRangeRead(keyspace,
                                            ...
                                            contacts,
                                            newVnodeCount,
                                            (newClusterMetadata) -> forRangeRead(newClusterMetadata, keyspace, consistencyLevel, newRange, newVnodeCount, false),
                                            (token) -> {
